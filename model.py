...
class ColaModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        outputs = self.forward(batch["input_ids"], batch["attention_mask"], labels=batch["label"])
        preds = torch.argmax(outputs.logits, 1)
        self.validation_step_outputs_labels = labels
        self.validation_step_outputs_logits = outputs.logits
    ...
    def on_validation_epoch_end(self):
        labels = self.validation_step_outputs_labels
        logits = self.validation_step_outputs_logits
        # preds = torch.argmax(logits, 1)

        ## There are multiple ways to track the metrics
        # 1. Confusion matrix plotting using inbuilt W&B method
        self.logger.experiment.log(
            {"conf": wandb.plot.confusion_matrix(probs=logits.numpy(), y_true=labels.numpy())}
        )
        ...
        del self.validation_step_outputs_labels  # free memory
        del self.validation_step_outputs_logits  # free memory
